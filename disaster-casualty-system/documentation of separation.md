...
This document explains the separation of the old combined responder setup into separate Field Responder and SAR Responder roles.
...
- `sa_responder` for SAR / Stabilization Area Responder accounts.
...
- SAR Responder -> `transport`
- HCFD -> `receiving_facility_ed`
- Legacy Responder -> `scene`

The responder/documenter export now includes the new FR and SAR roles, so exports do not fail or omit those accounts.

The API permission middleware already treats FR and SAR as responder-compatible roles, so existing responder endpoints continue to work.
...
When editing existing accounts, legacy `responder` still appears as `Legacy Responder` so old accounts can still be seen and reassigned to Field Responder or SAR Responder.
...
For compatibility, uploaded role labels like FR, Field Responder, SAR, Stabilization Area Responder, HCFD, Healthcare Documenter, and Documenter are normalized into the correct stored role.

## Mobile App Changes

The mobile app already had role-aware Add Casualty and Records behavior for FR, SAR, and HCFD. The transition tightened the role precedence:

- New `field_responder` accounts always use the Field Responder casualty flow.
- New `sa_responder` accounts always use the SAR/Stabilization casualty flow.
...
For new FR and SAR accounts, the mobile Profile page shows the assigned function but does not allow switching it from the device. Role assignment is now controlled by admin account management.

## Existing Accounts

Existing accounts with role `responder` are preserved.

They are not automatically converted because the system cannot safely know whether each old responder account should become FR or SAR.
...
The web dashboard still displays old records using the data already stored in the record, including Field Responder, SAR, and HCFD sections where data exists.
...
This avoids breaking current data while allowing the team to move forward with clearly separated FR, SAR, and HCFD accounts.

## Account Creation Validation Issue Found After Separation

After the role separation, creating a new FR or SAR account can still show the old security message:
...
2. Create a Field Responder or SAR account.
...
After separating FR, SAR, and HCFD into distinct account roles, each role can create a separate casualty record for the same real-world casualty.

This means the dashboard may show partial role sections:

- FR section filled, SAR and HCFD empty.
- SAR section filled, FR and HCFD empty.
- HCFD section filled, FR and SAR empty.

That is expected until a future Match Casing workflow links those separate role submissions into one casualty case.

Because the fields collected by FR, SAR, and HCFD are different, matching cannot rely on identical field values. The only commonly similar field is victim code, but victim code can differ because different users may encode or generate different values.
...
- SAR / Stabilization Area Responder: existing photo attachment remains in the Remarks step.
...
- Lets the admin fill one Field Responder slot, one SAR slot, and one HCFD slot.
- Lets the admin match the completed three-role selection into one locked casualty case.
...
- New matches require one Field Responder record, one SAR record, and one HCFD record.
...
- `POST /api/casualties/case-links`
  - Creates a complete matched casualty case from exactly three selected records: FR, SAR, and HCFD.
...
This gives admins a manual way to connect separate FR, SAR, and HCFD submissions while preserving the separated role records.
...
1. Open **Match Casing**.
2. Fill the three role boxes:
   - Field Responder
   - SAR
   - HCFD
3. Click **Match selected records** at the bottom of the three boxes.
...
7. The section lists the linked FR, SAR, and HCFD records and includes an **Open matched record** button.
8. Scroll to the role sections.
   - Linked FR data fills the Field Responder section.
   - Linked SAR data fills the Stabilization Area Responder section.
...
The Match Casing selection flow was changed from open record-card selection to a guided three-slot workflow.

Current behavior:

- The Match Casing page only shows available record counts and three required role boxes.
- The required boxes are Field Responder, SAR, and HCFD.
...
- The SAR picker only shows SAR records.
...
- The **Match selected records** button appears below the three boxes.
- The button is enabled only when all three role boxes are filled.
...
This keeps matching manual while preventing incomplete matches and accidental duplicate same-role matches.
