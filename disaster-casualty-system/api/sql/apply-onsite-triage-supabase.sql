...
ALTER TABLE public.casualty_triage_assessments
  ...
  ADD COLUMN IF NOT EXISTS responder_category varchar(50),
  ADD COLUMN IF NOT EXISTS calculated_category varchar(50),
  ADD COLUMN IF NOT EXISTS assessment_answers jsonb,
  ADD COLUMN IF NOT EXISTS algorithm_version varchar(50),
  ADD COLUMN IF NOT EXISTS is_over_triage boolean,
  ADD COLUMN IF NOT EXISTS is_under_triage boolean,
...
ALTER TABLE public.casualty_triage_assessments
  DROP CONSTRAINT IF EXISTS casualty_triage_assessments_category_check;
...
ALTER TABLE public.casualty_triage_assessments
  DROP CONSTRAINT IF EXISTS casualty_triage_assessments_stage_check;

ALTER TABLE public.casualty_triage_assessments
  ADD CONSTRAINT casualty_triage_assessments_stage_check
  CHECK (
    triage_stage IN (
      'on_site',
      'facility_arrival',
